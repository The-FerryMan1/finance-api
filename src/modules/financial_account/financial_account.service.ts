...
export namespace FinancialAccountService {
  export async function FinancialAccountOnboard(
    {
      accountName,
      accountType,
      currentBalance,
      institution,
    }: FinancialAccountModel.FinancialAccountBody,
    { userID }: userIDModel
  ) {
    const [row] = await db
    ...
    if (!row)
      throw status(
        400,
        "Unable to create a financial account" satisfies FinancialAccountModel.FinancialAccountInvalid
      );
  ...
  export async function ReadFinancialAccountById(
    ...
    if (isNaN(FinancialAccountIDConverted))
      throw status(
        400,
        "Parameter should be numeric" satisfies FinancialAccountModel.FinancialAccountParamsInvalid
      );
  ...
  export async function ModifyFinancialAccount(
    {
      accountName,
      accountType,
      currentBalance,
    ...
    if (checkRow === 0)
      throw status(404, "Financial account does not exists or access denied.");
    ...
    if (!row)
      throw status(500, "Database error: Faild to retrieve updated record.");
